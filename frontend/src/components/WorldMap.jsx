...
import countryGeoData from '../utils/countriesGeo.json';
...
// Função para estilizar os países no GeoJSON
const getCountryStyle = (feature) => {
  const countryCode = feature.properties.iso_a2;
  const countryData = hierarchicalData.find(c => c.id.toLowerCase() === countryCode.toLowerCase());
  
  return {
    fillColor: countryData ? typeColors[countryData.type] || '#CCCCCC' : '#CCCCCC',
    weight: 2,
    opacity: 1,
    color: 'white',
    dashArray: '3',
  ...
  const onEachCountry = (feature, layer) => {
    ...
    if (countryData) {
      layer.bindPopup(`
        ...
        <div>Tipo: ${typeLabels[countryData.type]}</div>
        <div>Doenças: ${countryData.diseaseCount}</div>
        <div>Medicamentos: ${countryData.medicineCount}</div>
      `);
    }
  };
